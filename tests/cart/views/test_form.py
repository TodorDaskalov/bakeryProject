...
class OrderFormTestCase(TestCase):
    ...
    # This test is accurate only in working hours of the bakery
    def test_form__pickup_time_before_current_time(self):
        current_time = datetime.now().time()
        form = OrderForm(data={'pickup_time': f'{9}:{00}'})
        if not (current_time < datetime.strptime('09:00', '%H:%M').time() or
                current_time > datetime.strptime('19:50', '%H:%M').time()):
